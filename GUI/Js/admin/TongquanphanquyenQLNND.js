...
async function PhanQuyen() {
       ...
       for (let item of dataFunction) {
              ...
              let checkBox_seePermission = document.getElementById(`${codePermission}-${item.functionCode}-seePermission`);
              let checkBox_addPermission = document.getElementById(`${codePermission}-${item.functionCode}-addPermission`);
              let checkBox_deletePermission = document.getElementById(`${codePermission}-${item.functionCode}-deletePermission`);
              let checkBox_fixPermission = document.getElementById(`${codePermission}-${item.functionCode}-fixPermission`);
       ...
       try {
              const response = await fetch('../../../BLL/ManagerUserGroupBLL.php', {
                     ...
                     body:
                            'function=' + encodeURIComponent('updatePermissionDetail') + '&arr=' + encodeURIComponent(JSON.stringify(arrObjPermissionDetail))
              });
              const data = await response.json();
              console.log(data);
