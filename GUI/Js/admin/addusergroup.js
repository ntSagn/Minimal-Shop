...
async function getDataFunction() {
  try {
    const response = await fetch('../../../BLL/ManagerUserGroupBLL.php', {
      ...
      body:
        'function=' + encodeURIComponent('getArrFunction')
    });
...
async function addPermissionDetail(codePermission) {
  ...
  try {
    const response = await fetch('../../../BLL/ManagerUserGroupBLL.php', {
      ...
      body:
        'function=' + encodeURIComponent('addArrPermisstionDetail') + '&arr=' + encodeURIComponent(JSON.stringify(arrObjPermissionDetail))
    });
    const data = await response.json();
    console.log(data);
